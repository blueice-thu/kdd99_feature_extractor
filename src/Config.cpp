...
namespace FeatureExtractor {
     ...
     */
    Config::Config()
            : files_c(0), files_v(nullptr), interface_num(1), pcap_read_timeout(1000), additional_frame_len(0),
              ipfrag_timeout(30), ipfrag_check_interval_ms(1000), tcp_syn_timeout(120),
              tcp_estab_timeout(5 * 24 * 3600) // 5 days
            , tcp_rst_timeout(10), tcp_fin_timeout(120), tcp_last_ack_timeout(30), udp_timeout(180), icmp_timeout(30),
              conversation_check_interval_ms(1000), time_window_size_ms(2000), count_window_size(100),
              print_extra_features(false), print_filename(false) {
    }
