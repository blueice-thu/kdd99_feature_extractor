...
namespace FeatureExtractor {
    ...
    TStatsPerHost *StatsWindow<TStatsPerHost, TStatsPerService>::find_or_insert_host_stats(uint32_t dst_ip) {
        ...
        if (it != per_host.end() && !(per_host.key_comp()(dst_ip, it->first))) {
            ...
            it = per_host.insert(it, typename map<uint32_t, TStatsPerHost>::value_type(dst_ip,
                                                                                       TStatsPerHost(feature_updater)));
            stats = &it->second;
        }
    ...
    template
    class StatsWindow<StatsPerHost, StatsPerService>;

    template
    class StatsWindow<StatsPerHost, StatsPerServiceWithSrcPort>;
}
